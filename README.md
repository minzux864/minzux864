...
const jadibot = async (Mikasa, text, m) => {
const { sendImage, sendMessage } = Mikasa;
...
Mikasa.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

Mikasa.ws.on('CB:call', async (json) => {
...
let qutsnya = await Mikasa.sendContact(callerId, owner)
await Mikasa.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await Mikasa.updateBlockStatus(callerId, "block")
}
})
...
m = smsg(Mikasa, kay, store)
require('./XeonCheems8.js')(Mikasa, m, chatUpdate, store)
} catch (err) {
console.log(err)}
})
...
Mikasa.ev.on("connection.update", async up => {
...
if (connection == "open") {
Mikasa.id = Mikasa.decodeJid(Mikasa.user.id)
Mikasa.time = Date.now()
global.conns.push(Mikasa)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${Mikasa.decodeJid(Mikasa.user.id)}*_`)
user = `${Mikasa.decodeJid(Mikasa.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); Mikasa.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Mikasa.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Mikasace Logged Out, Please Scan Again And Run.`); Mikasa.logout(); }
else if (reason === DisconnectReason.restartRequired) { 
console.log("Restart Required, Restarting..."); start(); }
else if (reason === DisconnectReason.timedOut) { 
console.log("Connection TimedOut, Reconnecting..."); start(); }
else Mikasa.end(`Unknown DisconnectReason: ${reason}|${connection}`)
}
})
...
for (let i of kon) {
list.push({
displayName: await Mikasa.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await Mikasa.getName(i + '@s.whatsapp.net')}\n
FN:${await Mikasa.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
Mikasa.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}
